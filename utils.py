...
def better_plotactions( plotFile, actions_per_episode):
    plt.plot(range(len(actions_per_episode)),actions_per_episode)
    plt.xlabel('Episodes')
    plt.ylabel('# Rewards')
...
def hash_matrix(board , do_fill):
    if (do_fill):
        board = fill_matrix(board)
...
def reverse_state(obs):
    ...
    for i in range(len(movements)):
        if movements[i] == 0:
            a = positions[str(i)]
            if obs['observation'][a[1]][a[0]][0] == 1:
                obs['observation'][a[1]][a[0]][0] =0
                obs['observation'][a[1]][a[0]][1] =1

            elif obs['observation'][a[1]][a[0]][1] == 1:
                obs['observation'][a[1]][a[0]][1] =0
                obs['observation'][a[1]][a[0]][0] =1
    return obs
